...
class Xui {
    ...
    public function __construct(
        ...
        $this->remote_address = trim($remote_address);
        $this->ws_host        = trim($ws_host);
    ...
    private function fetchRawConfig(string $sub_id, string $clean_sub): string {
        ...
        // Override remote address (host) – manual takes precedence over sub domain
        $override_host = $this->remote_address !== ''
            ? $this->stripScheme($this->remote_address)
            : (parse_url($clean_sub, PHP_URL_HOST) ?: '');
        if ($override_host) {
            $config = preg_replace('/(@)([^:\/?#]+)(:)/', '${1}' . $override_host . '${3}', $config);
        }

        // Override ws host header when set
        if ($this->ws_host !== '') {
            $config = $this->replaceQueryParam($config, 'host', $this->ws_host);
        }
        return $config;
    }
    ...
    private function replaceQueryParam(string $config, string $param, string $value): string {
        $parsed = parse_url($config);
        if (!isset($parsed['query']) || $parsed['query'] === '') {
            $sep = strpos($config, '?') === false ? '?' : '&';
            return preg_replace('/(#|$)/', $sep . $param . '=' . urlencode($value) . '$1', $config, 1);
        }
        parse_str($parsed['query'], $qs);
        $qs[$param] = $value;
        $new_query  = http_build_query($qs);
        return preg_replace('/\?[^#]*/', '?' . $new_query, $config, 1);
    }

    private function buildConfigFromInbound(?array $inbound_data, string $uuid, string $email, string $clean_sub): string {
        // ترتیب اولویت: remote_address دستی → دامنه ساب → هاست پنل
        if ($this->remote_address !== '') {
            $host = $this->stripScheme($this->remote_address);
        } else {
        ...
        if ($inbound_data && isset($inbound_data['success']) && $inbound_data['success'] && isset($inbound_data['obj'])) {
            ...
            if ($net === 'ws') {
                ...
                $params['path'] = $ws['path'] ?? '/';
                $params['host'] = $this->ws_host !== '' ? $this->ws_host : ($ws['headers']['Host'] ?? $host);
            } elseif ($net === 'grpc') {
