...
class AdminHandler {
    ...
    private function handleServerManagement(array $update, int $user_id, int $chat_id, string $text, ?string $step): bool {
        if ($text === '📋 لیست سرورها') {
            ...
            if (empty($servers)) {
                ...
                foreach ($servers as $s) {
                    ...
                    $detail   = $this->pdo->prepare("SELECT remote_address, ws_host FROM servers WHERE id = ?");
                    $detail->execute([(int)$s['id']]);
                    $extra    = $detail->fetch(PDO::FETCH_ASSOC) ?: [];
                    $remote   = !empty($extra['remote_address']) ? $extra['remote_address'] : '—';
                    $ws_host  = !empty($extra['ws_host']) ? $extra['ws_host'] : '—';
                    $msg .= "🖥 <b>{$s['name']}</b> (ID: <code>{$s['id']}</code>)\n"
                          . "🔗 آدرس: <code>{$s['url']}</code>\n"
                          . "🌐 Remote: <code>{$remote}</code>\n"
                          . "🪪 WS Host: <code>{$ws_host}</code>\n"
        ...
        if ($text === '✏️ ویرایش Remote/WS سرور') {
            setStep($this->pdo, $user_id, 'edit_srv_pick_id');
            $this->telegram->request('sendMessage', ['chat_id' => $chat_id, 'parse_mode' => 'HTML',
                'text' => "✏️ شناسه عددی سروری که می‌خواهید Remote Address یا WS Host آن را تنظیم کنید را وارد کنید:\n<i>(از «📋 لیست سرورها» می‌توانید شناسه را ببینید)</i>",
                'reply_markup' => getCancelKeyboard()]);
            return true;
        }
        if ($step === 'edit_srv_pick_id') {
            $srv_id = (int)onlyNumber($text);
            $stmt   = $this->pdo->prepare("SELECT id, name, remote_address, ws_host FROM servers WHERE id = ?");
            $stmt->execute([$srv_id]);
            $srv = $stmt->fetch(PDO::FETCH_ASSOC);
            if (!$srv) {
                $this->telegram->sendMessage($chat_id, "❌ سروری با این شناسه پیدا نشد.");
                return true;
            }
            $cur_remote = $srv['remote_address'] ?: '—';
            $cur_ws     = $srv['ws_host'] ?: '—';
            $keys = json_encode(['inline_keyboard' => [
                [['text' => '🌐 ویرایش Remote Address', 'callback_data' => "edit_srv_remote_{$srv['id']}"]],
                [['text' => '🪪 ویرایش WS Host',         'callback_data' => "edit_srv_wshost_{$srv['id']}"]],
            ]]);
            $this->telegram->request('sendMessage', ['chat_id' => $chat_id, 'parse_mode' => 'HTML',
                'text' => "🖥 <b>{$srv['name']}</b>\n🌐 Remote: <code>{$cur_remote}</code>\n🪪 WS Host: <code>{$cur_ws}</code>",
                'reply_markup' => $keys]);
            setStep($this->pdo, $user_id, 'admin_servers');
            return true;
        }
        if (strpos($step ?? '', 'edit_srv_remote_val|') === 0 || strpos($step ?? '', 'edit_srv_wshost_val|') === 0) {
            $is_remote = strpos($step, 'edit_srv_remote_val|') === 0;
            $col       = $is_remote ? 'remote_address' : 'ws_host';
            $srv_id    = (int)explode('|', $step)[1];
            $value     = trim($text);
            if ($value === '-') $value = '';
            $this->pdo->prepare("UPDATE servers SET $col = ? WHERE id = ?")->execute([$value, $srv_id]);
            setStep($this->pdo, $user_id, 'admin_servers');
            $label = $is_remote ? 'Remote Address' : 'WS Host';
            $this->telegram->request('sendMessage', ['chat_id' => $chat_id, 'parse_mode' => 'HTML',
                'text' => "✅ {$label} سرور شماره <code>{$srv_id}</code> بروزرسانی شد.",
                'reply_markup' => getServersKeyboard()]);
            return true;
        }
    ...
    private function handleServerSteps(int $user_id, int $chat_id, string $text, ?string $step): bool {
        ...
        if (strpos($step ?? '', 'add_srv_inbound|') === 0) {
            ...
            $this->telegram->request('sendMessage', ['chat_id' => $chat_id, 'parse_mode' => 'HTML',
                'text' => "دامنه Sub (اختیاری، برای رد شدن «-» بفرستید):\n<i>مثلاً: https://sub.example.com</i>",
                'reply_markup' => getCancelKeyboard()]);
            return true;
        }
        if (strpos($step ?? '', 'add_srv_sub|') === 0) {
            $parts      = explode('|', $step);
            $sub_domain = (trim($text) === '-' || trim($text) === '') ? '' : trim($text);
            setStep($this->pdo, $user_id, "add_srv_remote|{$parts[1]}|{$parts[2]}|{$parts[3]}|{$parts[4]}|{$parts[5]}|" . str_replace('|', '/', $sub_domain));
            $this->telegram->request('sendMessage', ['chat_id' => $chat_id, 'parse_mode' => 'HTML',
                'text' => "🌐 <b>Remote Address</b> دستی (اختیاری، «-» برای رد):\n<i>این مقدار جایگزین <code>@host</code> در کانفیگ می‌شود.\nمثلاً: <code>de1.myserver.com</code> یا <code>1.2.3.4</code></i>",
                'reply_markup' => getCancelKeyboard()]);
            return true;
        }
        if (strpos($step ?? '', 'add_srv_remote|') === 0) {
            $parts          = explode('|', $step);
            $remote_address = (trim($text) === '-' || trim($text) === '') ? '' : trim($text);
            setStep($this->pdo, $user_id, "add_srv_wshost|{$parts[1]}|{$parts[2]}|{$parts[3]}|{$parts[4]}|{$parts[5]}|{$parts[6]}|" . str_replace('|', '/', $remote_address));
            $this->telegram->request('sendMessage', ['chat_id' => $chat_id, 'parse_mode' => 'HTML',
                'text' => "🪪 <b>WS Host</b> دستی (اختیاری، «-» برای رد):\n<i>مقدار هدر Host برای WebSocket. مثلاً: <code>cdn.cloudflare.com</code></i>",
                'reply_markup' => getCancelKeyboard()]);
            return true;
        }
        if (strpos($step ?? '', 'add_srv_wshost|') === 0) {
            $parts      = explode('|', $step);
            $ws_host    = (trim($text) === '-' || trim($text) === '') ? '' : trim($text);
            $next_pri   = (int)$this->pdo->query("SELECT COALESCE(MAX(priority),0)+1 FROM servers")->fetchColumn();
            $this->pdo->prepare(
                "INSERT INTO servers (name, url, user, pass, inbound_id, sub_domain, remote_address, ws_host, is_active, priority)
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1, ?)"
            )->execute([
                $parts[1], $parts[2], $parts[3], $parts[4],
                (int)$parts[5], $parts[6], $parts[7], $ws_host, $next_pri,
            ]);
            setStep($this->pdo, $user_id, 'admin_servers');
            $summary = "✅ سرور «{$parts[1]}» اضافه شد.";
            if ($parts[7] !== '') $summary .= "\n🌐 Remote: <code>{$parts[7]}</code>";
            if ($ws_host !== '')  $summary .= "\n🪪 WS Host: <code>{$ws_host}</code>";
            $this->telegram->request('sendMessage', ['chat_id' => $chat_id, 'text' => $summary,
                'parse_mode' => 'HTML', 'reply_markup' => getServersKeyboard()]);
            return true;
        }
        if ($step === 'test_srv_id') {
            $srv_id = (int)onlyNumber($text);
    ...
    private function handlePanelSettings(int $user_id, int $chat_id, string $text, ?string $step): bool {
        $panel_map = [
            '🔗 تغییر آدرس پنل'        => 'set_panel_url',
            '👤 تغییر یوزرنیم'          => 'set_panel_user',
            '🔑 تغییر پسورد'           => 'set_panel_pass',
            '🆔 تغییر آیدی کانفیگ'      => 'set_inbound_id',
            '🌐 تنظیم دامنه لینک ساب'   => 'set_sub_domain',
            '🌐 تنظیم Remote Address'  => 'set_remote_address',
            '🪪 تنظیم WS Host'         => 'set_ws_host',
        ];
        if (array_key_exists($text, $panel_map)) {
            setStep($this->pdo, $user_id, $panel_map[$text]);
            $hint = '';
            if ($panel_map[$text] === 'set_remote_address') {
                $cur = $this->settings['remote_address'] ?? '';
                $hint = "🌐 <b>Remote Address پیش‌فرض</b>\nمقدار فعلی: <code>" . ($cur !== '' ? $cur : '—') . "</code>\n\nمقدار جدید را وارد کنید («-» برای پاک کردن):";
            } elseif ($panel_map[$text] === 'set_ws_host') {
                $cur = $this->settings['ws_host'] ?? '';
                $hint = "🪪 <b>WS Host پیش‌فرض</b>\nمقدار فعلی: <code>" . ($cur !== '' ? $cur : '—') . "</code>\n\nمقدار جدید را وارد کنید («-» برای پاک کردن):";
            } else {
                $hint = 'مقدار جدید:';
            }
            $this->telegram->request('sendMessage', ['chat_id' => $chat_id, 'text' => $hint,
                'parse_mode' => 'HTML', 'reply_markup' => getCancelKeyboard()]);
            return true;
        }
        if ($step && (strpos($step, 'set_panel_') === 0 || in_array($step, ['set_inbound_id', 'set_sub_domain', 'set_remote_address', 'set_ws_host']))) {
            $key   = str_replace('set_', '', $step);
            $value = trim($text);
            if ($value === '-') $value = '';
            updateSetting($this->pdo, $key, $value);
